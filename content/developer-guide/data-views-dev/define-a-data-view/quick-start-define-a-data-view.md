...
# Define a data view
The `DataView` object is a declarative query and shape for OCS data. This section describes the `DataView` object. See the <xref:data-views-data-views> for the corresponding API routes.

At times, this section makes reference to ways the view *resolves* into further resources, such as the collection of data items found by each data item query. See <xref:ResolvedDataView> and <xref:data-views-data-views-resolved> sections for details.
